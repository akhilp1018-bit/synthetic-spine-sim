...
def _sample_fov_origin_from_dendrite(
    dendrite_path,
    rotation,
    scale_to_nm,
    patch_size_px,
    ...
    """Sample a random FOV center from the rotated dendrite and return origin_nm."""
...
def generate_random_transform(
    ...
    4. Keep only spines whose center falls inside the FOV
    5. Retry until at least min_spines_in_fov are inside, or max_tries reached
    6. Save transformed meshes to temp files

    Returns
    -------
    dict with all parameters needed for rendering plus temp mesh paths.
    """
    rng = np.random.default_rng(seed)
    ...
    for _ in range(max_tries):
        ...
        origin_nm = _sample_fov_origin_from_dendrite(
        ...
        result = {
            ...
            "rotation": rotation,
            "seed": seed,
            "num_spines_in_fov": len(sim_spine_paths_inside),
        }

        last_result = result

        if len(sim_spine_paths_inside) >= int(min_spines_in_fov):
            print(
                f"  Sample={sample_name}  XY={xy_nm_per_px:.1f} nm/px  "
                f"Spines inside FOV: {len(sim_spine_paths_inside)} / {len(spine_paths)}"
            )
            return result

        cleanup_temp_meshes(result)

    if last_result is None:
        raise RuntimeError("Failed to generate random transform.")

    print(
        f"  Warning: could not reach min_spines_in_fov={min_spines_in_fov}. "
        f"Using last attempt with {last_result['num_spines_in_fov']} spines in FOV."
    )
    return last_result
