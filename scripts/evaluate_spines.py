...
def distance_matrix_gpu(pA, pB, dx=94.0, dy=94.0, dz=500.0):
    """
    Compute distance matrix between two sets of 3D points using GPU.
    pA: (N, 3) array of GT spine locations (X, Y, Z)
    pB: (M, 3) array of predicted locations (X, Y, Z)
    Returns: (N, M) distance matrix in nm
    """
...
def compute_iou_dice_gpu(gt_binary, pred_prob, threshold):
    ...
    Uses GPU for fast computation on large volumes.
    """
...
for model_name, spine_prob_path in MODELS.items():
    ...
    for thresh in THRESHOLDS:
        ...
        if len(peak_coords) == 0:
            ...
            # Match GT to nearest prediction
    ...
    ap       = float(np.trapz(prec_arr[sort_idx], rec_arr[sort_idx]))
...
for ax, title, ylabel in zip(axes,
    ["IoU vs Threshold — Spine", "Dice Score vs Threshold — Spine"],
    ["IoU", "Dice Score"]):
