...
print(f"DENDRITE_MASK_REL_THRESHOLD={DENDRITE_MASK_REL_THRESHOLD}")
print(f"SAVE_DEBUG_COMPONENTS={SAVE_DEBUG_COMPONENTS}")
print("===============")
...
def build_density_for_mesh(mesh_path, spacing_nm, tag):
    print("\n" + "=" * 60)
...
for spacing_nm in SPACING_LIST_NM:
    print("\n" + "=" * 60)
    print(f"Running labeled submesh experiment: spacing_nm = {spacing_nm}")
    print("=" * 60)

    rho_dendrite = build_density_for_mesh(sim_dendrite_path, spacing_nm, "dendrite")

    rho_spines_list = []
    for i, spine_path in enumerate(sim_spine_paths, start=1):
        rho_sp = build_density_for_mesh(spine_path, spacing_nm, f"spine_{i}")
    ...
    base_tag = (
        f"labeled_{LABELING_MODE}_{psf_tag}_spacing{int(spacing_nm)}nm"
    )
